...
def finding(source_id):
    ...
    key = next(iter(source["claims"]))
    claim = source["claims"][key]
    return {
        "source_id": source_id,
        "key": key,
...
def report(topic, ids=(), status="ok"):
    return {
        "topic": topic,
        "status": status,
        "attempts": [{"query": "Read dated " + topic + " sources", "outcome": status}],
        "findings": [finding(i) for i in ids],
        "error": {
            "type": "timeout",
            "retryable": True,
            "alternatives": [
                "Retry this source later or ask a human for the policy copy."
            ],
        }
        if status == "error"
        else None,
