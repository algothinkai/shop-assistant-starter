...
OLD = "POL-RETURN-2026-06"
NEW = "POL-RETURN-2026-09"
FAQ = "FAQ-RETURNS-2026-09"
SHIP = "POL-SHIP-2026-06"
...
class ResearchChecks(unittest.TestCase):
    ...
    def test_current_disagreement_preserves_both_attributed_values(self):
        value = self.run_case({"returns": report("returns", [NEW, FAQ])})["topics"][
            "returns"
        ]
    ...
    def test_superseded_source_is_history_not_current_conflict(self):
        ...
        value = self.run_case(
            {"returns": report("returns", [OLD, NEW])}, sources=sources
        )["topics"]["returns"]
    ...
    def test_future_sources_do_not_change_past_policy(self):
        value = self.run_case(
            {"returns": report("returns", [OLD, NEW, FAQ])}, as_of="2026-08-15"
        )["topics"]["returns"]
        self.assertEqual(value["coverage"], "supported")
    ...
    def test_missing_current_source_creates_targeted_follow_up(self):
        value = self.run_case({"returns": report("returns", [NEW])})["topics"][
            "returns"
        ]
    ...
    def test_valid_empty_differs_from_access_failure(self):
        empty = self.run_case({"returns": report("returns", status="empty")})["topics"][
            "returns"
        ]
        failed = self.run_case({"returns": report("returns", status="error")})[
            "topics"
        ]["returns"]
    ...
    def test_partial_error_preserves_findings_and_healthy_sibling(self):
        result = self.run_case(
            {
                "returns": report("returns", [NEW], "error"),
                "shipping": report("shipping", [SHIP]),
            },
    ...
    def test_forged_quote_or_cross_topic_result_is_not_accepted(self):
        for changed in [
            dict(quote="shortened evidence"),
            dict(value=True),
            dict(source_id=SHIP),
        ]:
            item = report("returns", [NEW])
            item["findings"][0].update(changed)
    ...
    def test_invalid_sibling_does_not_destroy_valid_work(self):
        result = self.run_case(
            {"returns": {"status": "ok"}, "shipping": report("shipping", [SHIP])},
            topics=("returns", "shipping"),
        )
    ...
    def test_empty_cannot_carry_findings_or_hide_failure(self):
        for item in [
            report("returns", [NEW], "empty"),
            {**report("returns", status="error"), "status": "empty"},
        ]:
            self.assertEqual(
                self.run_case({"returns": item})["topics"]["returns"]["coverage"],
                "invalid_evidence",
            )

    def test_input_reports_and_catalog_are_not_mutated(self):
        item = {"returns": report("returns", [OLD, NEW, FAQ])}
        original = copy.deepcopy(item)
        catalog = copy.deepcopy(SOURCES)
        result = self.run_case(item)
        result["topics"]["returns"]["claims"][0]["value"] = 999
        self.assertEqual(item, original)
        self.assertEqual(SOURCES, catalog)

    def test_every_current_claim_is_required_not_only_source_presence(self):
        sources = copy.deepcopy([s for s in SOURCES if s["id"] == SHIP])
        sources[0]["claims"]["tracking_review"] = {
            "value": "Required",
            "quote": "Teaching regression: review absent tracking.",
        }
        reports = {"shipping": report("shipping", [SHIP])}
        result = self.run_case(reports, topics=("shipping",), sources=sources)
        self.assertFalse(result["ready"])
        self.assertEqual(result["topics"]["shipping"]["coverage"], "gap")
        self.assertEqual(
            result["topics"]["shipping"]["missing_claims"],
            [{"source_id": SHIP, "key": "tracking_review"}],
        )
        reports["shipping"]["findings"].append(
            {
                "source_id": SHIP,
                "key": "tracking_review",
                **sources[0]["claims"]["tracking_review"],
        ...
        self.assertTrue(
            self.run_case(reports, topics=("shipping",), sources=sources)["ready"]
        )

    def test_transitive_supersession_does_not_revive_older_claim(self):
        base = copy.deepcopy(next(s for s in SOURCES if s["id"] == OLD))
        ...
        middle["claims"]["return_window_days"] = {
            "value": 45,
            "quote": "Fictional middle version.",
        }
        ...
        latest["claims"]["return_window_days"] = {
            "value": 60,
            "quote": "Fictional latest version.",
        }
        sources = [base, middle, latest]
        item = report("returns", [OLD])
        item["findings"] = [
            {
                "source_id": s["id"],
                "key": "return_window_days",
                **s["claims"]["return_window_days"],
            }
            for s in sources
        ]
        value = self.run_case({"returns": item}, sources=sources)["topics"]["returns"]
        self.assertEqual(value["coverage"], "supported")
        self.assertEqual([c["source_id"] for c in value["claims"]], ["LATEST"])
        self.assertEqual([c["source_id"] for c in value["historical"]], [OLD])
        self.assertEqual([c["source_id"] for c in value["out_of_scope"]], ["MID"])
